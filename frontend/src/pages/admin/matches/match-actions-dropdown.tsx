...
import { MatchImportDialog } from './match-import-dialog';
...
export function MatchActionsDropdown({ match, teamNames, onChanged, onForceResult }: {
  ...
  async function executeSwap() {
    setSubmitting(true);
    try {
      await api.swapMatchSides(match.id);
      toast.success('Home/away swapped');
      setSwapOpen(false);
      onChanged();
    } catch (err: unknown) {
      toast.error(getErrorMessage(err));
  ...
  return (
    <>
      ...
      <Dialog open={swapOpen} onOpenChange={setSwapOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {`${homeName} and ${awayName} will switch sides (W${match.week}). Scores and bracket seeds move with them; the winner is unchanged. Activity log will record the change.`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={executeSwap}
            >
              {submitting ? 'Swapping...' : 'Swap sides'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {/* Attach Played Battle / Replace Result */}
      <MatchImportDialog
        match={match}
        homeName={homeName}
        awayName={awayName}
