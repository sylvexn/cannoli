...
export const teamRoutes = new Elysia()
  ...
  .post('/api/leagues/:leagueId/free-agents/pickup', ({ params, query, body, user, set }) => {
    ...
    // All pickups — staff and non-staff alike (feedback #73) — are queued for
    // admin approval (feedback #42): validate against the live rules with a dry
    // run (no mutation), park the request as pending, and return. An admin
    // applies it later via the approve endpoint (self-approval is allowed —
    // any admin, including the initiator, may approve in a second explicit step).
